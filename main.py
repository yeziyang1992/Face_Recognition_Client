...
class MyDesignerShow(QtWidgets.QWidget, Ui_widget):
    ...
    def __init__(self):
        ...
        self.names = self.face_recog.names
    ...
    def get_local_camera(self):
        if self.timer_udp_video.isActive():      # 查询网络摄像头
    ...
    def debug(self):
        ...
        file_names = self.face_recog.pic_names
        print(file_names)
        if num > 0:
            result, ok = QtWidgets.QInputDialog.getItem(self, u"人脸数据校验", u"把人脸数据存入对应的文件夹中，可增加人脸识别的准确性。确定把图片存放在以下文件夹中吗？",
                                                        file_names, 1, False)
            if ok:
                if self.pic_pre is not None:
                    # 保存图片
                    s_time = time.ctime().replace(' ', '_').replace(':', '_')
                    print(s_time)
                    cv2.imwrite('./faces/' + result + '/' + str(s_time) + '.jpg', self.pic_pre)
                    self.textEdit.append(result + ' 保存成功')
        else:
